...
##            try:
##               register_cl_12_1[idx,:]=[tau1*math.exp(-U11 + f_12_1*fit_param*delta1),ch_1,ch_2,ch_3,ch_4,cl_1,cl_2,cl_3,cl_4] # track bothe the chains and the corresponding connected crosslinker
##            except UnboundLocalError:
##               print('len(crosslinker_array[idx].chains)',len(crosslinker_array[idx].chains))
##               print('k',k)
##               stop
##            register_cl_12_2[idx,:]=[tau1*math.exp(-U12 + f_12_2*fit_param*delta1),ch_1,ch_2,ch_3,ch_4,cl_1,cl_2,cl_3,cl_4]
...
##        broken_cl_bonds_register = np.zeros((n_bonds,3)) # the last 2 columns contain the 
        ...
        if(index_orig%10==0):
           ...
           file2.write('#type, atom1, atom2, length, rate(v), t, , vmax, active bonds\n') 
...
##        vmax_12=max(max(register_cl_12_1[:,0]),max(register_cl_12_2[:,0])) # 2*n_links
        ...
        vmax=max(vmax_chain,vmax_13)#vmax_broken_cl_chain,vmax_12,vmax_13)
##        stop
        '''
...
##            print('i',i)
        ...
        n_bonds=len(self.bonds[:,0]) 
        n_atoms=len(self.atoms[:,0])
        num_active_bonds=len(active_bonds[0])+sum_n_i+4*n_atoms
        
        t = 1/(vmax*len(active_bonds[0])) 
        print('KMC statistics:') 
        print('Max rate, Active bonds, and  = %6.4E, %5d, %6.4E'%(vmax, len(active_bonds[0]), t))
        if(t < delta_t):
##           print('##############################################')
...
##                n_bonds=len(self.bonds[:,0]) 
                n_atoms=len(self.atoms[:,0])
                num_active_bonds=len(active_bonds[0])+sum_n_i+4*n_atoms #+2*n_atoms
...
##                   t = t + t_KMC
                   ...
                   # In the case of control- there is only 1,3 regiochemistry or PEG chain breaking,
                   # hence, there is no way a broken crosslinker part can be there in the chain
                   # So, this case cannot happen.
                   # So, stop if this case gets chosen-0 just as a debug statement
...
########
                ...
                elif(int(bond_index/(len(active_bonds[0])+sum_n_i+4*n_atoms))==0): # crosslinker bond is broken based on 1,3 regiochemistry
